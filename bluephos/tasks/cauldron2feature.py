...
def make_cauldron(df_mols: pd.DataFrame) -> dict:
    ...
    cauldron_df = {"molecule_table": molecule_table, "one_atom_table": one_atom_table, "two_atom_table": two_atom_table}
    
    return cauldron_df

def join_feature(cauldron_df: dict, input_folder, output_folder) -> pd.DataFrame:

    element_features = pd.read_csv(os.path.join(input_folder, "element_features.csv"), dtype={"symbol": str})
    train_stats = pd.read_csv(os.path.join(input_folder, "train_stats.csv"), dtype={"feature": str})
    ...
    synthetic_two_atom = cauldron_df['two_atom_table'][["mol_id", "bond_id", "start_atom", "end_atom"]]

    # Scale features
    for feature in train_stats["feature"]:
        synthetic_one_atom[feature] = (synthetic_one_atom[feature] - train_stats.loc[train_stats["feature"]==feature, "center"].values[0]) / train_stats.loc[train_stats["feature"]==feature, "scale"].values[0]
    ...
    synthetic_mols.to_csv(os.path.join(output_folder, "featurized_mol_tbl.csv.gz"), index=False)

    # Write featurized atom table
    synthetic_one_atom.to_csv(os.path.join(output_folder, "featurized_one_tbl.csv.gz"), index=False)

    # Write featurized bond table
    synthetic_two_atom[["mol_id", "bond_id", "start_atom", "end_atom"]].to_csv(os.path.join(output_folder, "featurized_two_tbl.csv.gz"), index=False)
...
Cauldron2FeatureTask = PipelineTask('cauldron2feature', feature_create, context_kwargs={
