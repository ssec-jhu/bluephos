...
class Molecule:
    ...
    def get_positions(self):
        return self.one_atom_table[
            [
                self.special_colnames["x"],
                self.special_colnames["y"],
    ...
    def get_rdkit(self, sanitize=True, removeHs=True):
        ...
        one_tbl_column_types = non_special_column_types(
            self.get_one_atom_table(), self.special_colnames.keys()
        )
        ...
        for rowtuple in self.get_two_atom_table().itertuples():
            ...
            mol_reconstructed_editable.AddBond(
                start_atom_index, end_atom_index, order_rdkit
            )
        ...
        if (
            x_col in self.get_one_atom_table().columns
            and y_col in self.get_one_atom_table().columns
            and z_col in self.get_one_atom_table().columns
        ):
            ...
            for i, rowtuple in enumerate(self.get_one_atom_table().itertuples()):
                # Converting to a dictionary so I can access values by with square brackets
                row = rowtuple._asdict()
                conf.SetAtomPosition(i, Point3D(row[x_col], row[y_col], row[z_col]))
        ...
        two_tbl_column_types = non_special_column_types(
            self.get_two_atom_table(), self.special_colnames.keys()
        )
        for rowtuple in self.get_two_atom_table().itertuples():
            # Converting to a dictionary so I can access values by with square brackets
            row = rowtuple._asdict()
            ...
            start_atom_index = atom_indices[start_atom]
            end_atom_index = atom_indices[end_atom]
            bond = mol_reconstructed.GetBondBetweenAtoms(
                start_atom_index, end_atom_index
            )
            if bond is None:
                bond = mol_reconstructed.GetBondBetweenAtoms(
                    end_atom_index, start_atom_index
                )
        ...
        mol_tbl_column_types = non_special_column_types(
            self.get_molecule_table(), self.special_colnames.keys()
        )
    ...
    def get_ase(self):
        ...
        if not has_ase:
            raise ImportError(
                "Atomic simulation environment Python library required to return ase objects"
            )
...
def rdkit2cauldronoid(rdkit_mol):
    ...
    if rdkit_mol.HasProp("_Name") and rdkit_mol.GetProp("_Name") != "":
        ...
        name = None
    cauldronoid_mol = Molecule(
        molecule_table(rdkit_mol), atom_table(rdkit_mol), bond_table(rdkit_mol), name
    )
...
def bind_rows_map(table_fun, name_fun, id_colname, xs):
    ...
    return (
        concat(tables, axis=0, keys=names)
        .reset_index(level=0)
...
def tables2data(mol_id, molecule_table=None, atom_table=None, bond_table_oneway=None):
    ...
    if not has_torch:
        raise ImportError(
            "Cannot convert to Pytorch Geometric data because it is not installed"
        )
    output = Data()
...
def files2mols(
    directory=None,
    prefix=None,
    suffixes=("_mol_tbl.csv.gz", "_one_tbl.csv.gz", "_two_tbl.csv.gz"),
    molecule_table_path=None,
    one_atom_table_path=None,
    two_atom_table_path=None,
):
