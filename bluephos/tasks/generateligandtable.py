...
def generate_ligand_table(inputs: pd.DataFrame, halides_file_name, acids_file_name, output_folder) -> pd.DataFrame:
    
    halide_df = pd.read_csv(halides_file_name)
    acid_df = pd.read_csv(acids_file_name)
    ...
    result_df.to_csv(os.path.join(output_folder, "combinatorial_ligands.csv"), index=False)

    return result_df


GenerateLigandTableTask = PipelineTask('generate_ligand_table', generate_ligand_table,
    context_kwargs={
